"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var http_1 = require('@angular/http');
require('rxjs/Rx');
var UserProductsService = (function () {
    function UserProductsService(http) {
        this.http = http;
    }
    UserProductsService.prototype.getListOfCategories = function () {
        return this.http.get('/homeaction/GetCategoriesList')
            .map(function (response) { return response.text(); }).toPromise();
    };
    UserProductsService.prototype.getProductsPerPage = function (category) {
        return this.http.post('/homeaction/GetProducts', category)
            .map(function (data) { return data.text(); }).toPromise();
    };
    UserProductsService.prototype.getAdminProductsPerPage = function (category) {
        return this.http.post('/adminaction/GetProducts', category)
            .map(function (data) { return data.text(); }).toPromise();
    };
    UserProductsService.prototype.getAdminAllowToPublic = function (item) {
        return this.http.post('/adminaction/UpdateProductImage', item)
            .map(function (data) { return data.text(); }).toPromise();
    };
    UserProductsService.prototype.getUserFriendProducts = function (pageNumber) {
        return this.http.post('/accountaction/GetUserFriendProducts', pageNumber)
            .map(function (response) { return response.text(); }).toPromise();
    };
    UserProductsService.prototype.getUserProducts = function (pageNumber) {
        return this.http.post('/accountaction/GetUserProducts', pageNumber)
            .map(function (data) { return data.text(); }).toPromise();
    };
    UserProductsService.prototype.removeProductAdm = function (product) {
        return this.http.post('/accountaction/RemoveProductItem', product)
            .map(function (response) { return response.text(); }).toPromise();
    };
    UserProductsService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [http_1.Http])
    ], UserProductsService);
    return UserProductsService;
}());
exports.UserProductsService = UserProductsService;
